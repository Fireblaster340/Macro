...
try:
    ...
    showerror("Fatal Error","Fatal Error, pynput library failed to install, try installing manually... check help for more info")
    paused = True
import threading 
import time
root = tk.Tk()
...
Running = True
makenewremap = False
os.makedirs(os.path.join(os.path.expanduser("~"),"Desktop","Macro"),exist_ok=True)
...
def Settings():
    ...
    PauseB.pack(pady=3)
    CleanInfo = tk.Label(root, text="Cleanup recorded files")
    CleanInfo.pack(pady=3)
    CleanEmpty = tk.Button(root,text="Cleanup",command=Cleanup)
    CleanEmpty.pack(pady=3)
...
def Cleanup():
    root.withdraw()
    minsz = simpledialog.askinteger("SizeMin","Cleanup will remove all recorded macro files below a certain size. \nThis is helpful if you have recorded some macros that failed to record. \nWhat should be the minimum deletion size (in bytes)? ")
    if minsz is None:
        showerror("Error", "Enter some integer")
        return 0
    if not askokcancel("Confirm","Are you sure you want to lose a (potentially large) number of files?"):
        root.wm_deiconify()
        return 0
    try: 
        ...
        for i in contents:
            if os.path.getsize(i) < minsz:
                Delete(i,False)
    except FileNotFoundError:
        showerror("Error!", "Something went wrong with finding the pathnames file")
    root.wm_deiconify()
...
def Advanced():
    ...
    remapbutton = tk.Button(root,text="Remap Keys", command=RemapPage)
    remapbutton.pack(pady=6)
...
def NewFileRemap():
    global makenewremap
    makenewremap = not makenewremap
def RemapPage():
    global savedcontents, makenewremap
    ClearAll()
    root.geometry("400x450")
    title = tk.Label(root,text="Remap Keys",font=("TkDefaultFont",30))
    title.pack()
    info = tk.Label(root,text="Remapping a file replaces all instances of a key with another key.",wraplength=420)
    info.pack(pady=5)
    kvar1 = tk.StringVar(root)
    if len(savedcontents) != 0:
        k1 = tk.OptionMenu(root,kvar1,*savedcontents)
    else:
        k1 = tk.OptionMenu(root,kvar1,*["No saves... yet..."])
    k1.pack(pady=5)
    keyinfo = tk.Label(root,text="Key to replace",wraplength=420)
    keyinfo.pack(pady=5)
    keych = tk.Entry(root,width=5)
    keych.pack()
    keyreminfo = tk.Label(root,text="Key to replace with",wraplength=420)
    keyreminfo.pack(pady=5)
    keyrem = tk.Entry(root,width=5)
    keyrem.pack()
    newfileinfo = tk.Label(root,text="Create new file after remap?")
    newfileinfo.pack(pady=5)
    newfile = tk.Checkbutton(root,command=NewFileRemap)
    newfile.pack(pady=5)
    if makenewremap:
        newfile.select()
    submit = tk.Button(root,text="Remap",command=lambda:Remap(keych.get(),keyrem.get(),kvar1.get()))
    submit.pack(pady=5)
...
def Remap(ent1:str,ent2:str,file:str):
    if len(ent1) != 1 or len(ent2) != 1:
        showerror("Presence/Length","Please enter a single character key in each entry box")
        return 0
    try:
        fil = open(file,"r")
    except (FileNotFoundError):
        showerror("Input error", "File not selected or invalid file")
        return 0
    contents = []
    for line in fil.readlines():
        contents.append(line)
    fil.close()
    contents = list(map(RemoveBreak,contents))
    sample = contents[0]
    contents.pop(0)
    newcont = ""
    for x in range(len(contents)):
        newcont = ParseLine(contents[x])
        
        for j in range(len(newcont[3])):
            if newcont[3][j] == "'"+ent1+"'":
                newcont[3][j] = "'"+ent2+"'"
        ...
        for i in newcont[3]:
            chars = chars+i+" "
        newcont = f"{newcont[0][0]} {newcont[0][1]} {newcont[1][0]} {newcont[1][1]} {chars} M {mouse}"
                
        contents[x] = newcont
    try:
        if makenewremap:
            file =  NextFreePath(os.path.join(recordedpath, "Remap"))
        fil = open(file,"w")
        fil.close()
        fil = open(file,"w")
        fil.writelines(sample+"\n")
        for i in contents:
            fil.writelines(i+"\n")
        fil.close()
    except FileNotFoundError:
        showerror("Missing file", "A missing file was detected")
        return 0
    except OSError:
        showerror("OS error", "An unexpected OS error has occurred")
        return 0
    try:
        fil = open(pathnames,"a")
        fil.writelines(file+"\n")
        fil.close()
    except FileNotFoundError:
        showerror("Missing file", "Pathnames file is missing, attempting fix...")
        ReplayFiles()
        return 0
    showinfo("Success!", "Successfully remapped keys!")
...
def Delete(path:str, conf:bool=True):
